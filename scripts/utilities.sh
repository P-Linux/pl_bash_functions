...
ut_is_yes_no_var_abort() {
    ...
    (( $# < 3 )) &&  ms_abort "$_fn" "$(gettext "FUNCTION: '%s()' Requires AT LEAST '3' arguments. Got '%s'")" "$_fn" "$#"
...
ut_is_str_var_abort() {
    ...
    (( $# < 2 )) &&  ms_abort "$_fn" "$(gettext "FUNCTION: '%s()' Requires AT LEAST '2' arguments. Got '%s'")" "$_fn" "$#"
    [[ -n $1 ]] || ms_abort "$_fn" "$(gettext "FUNCTION Argument 1 MUST NOT be empty.")"
    [[ -n $2 ]] || ms_abort "$_fn" "$(gettext "FUNCTION Argument 2 MUST NOT be empty.")"
    local _var_name=$1
    ...
    if ! ut_is_str_var "$_var_name"; then
        if [[ -n $_extra_info ]]; then
            ...
            ms_abort "$_fn" "$(gettext "FUNCTION: '%s()' Not a declared string variable: '%s'")" "$_caller_name" "$_var_name"
        fi
    fi
}
...
ut_ref_associative_array_abort() {
    ...
    if [[ ! -n $_line ]]; then
        ms_abort "$_fn" "$(gettext "FUNCTION: '%s()' Not a referenced associative array: '%s'")" "$_caller_name" "$_ref_name"
    fi
...
ut_search_cmd_option_values_string() {
    ...
    if [[ ! -n $_short_arg && ! -n $_long_arg ]]; then
        ms_abort "$_fn" "$(gettext "WRONG CODE: Function: '%s()' Argument 1: '%s' Argument 2: '%s'. Only one MAY be empty.")" \
         "$_fn" "$_short_arg" "$_long_arg"
    fi
    (( _maximum_expected_values == 0 )) && ms_abort "$_fn" "$(gettext "Argument '_maximum_expected_values' MUST NOT be 0")"
    ...
    if [[ -n $_short_arg ]]; then
        ...
        if (( _check_arg_length < 2 )); then
            ...
            ms_abort "$_fn" "$(gettext "Short option to check: '%s' MUST start with EXACT ONE hyphen-minus.")" "$_short_arg"
        fi
    fi
    ...
    if [[ -n $_long_arg ]]; then
        ...
        if (( _check_arg_length < 3 )); then
            ms_abort "$_fn" "$(gettext "Long option to check: '%s' MUST be at least 3 character long or empty.")" "$_long_arg"
