...
ex_extract_only_copy() {
    ...
    if [[ ! -e $_destpath ]]; then
        ms_abort_remove_path "$_fn" "$_remove_build_dir" "$_build_srcdir" "$(gettext "File copy source  not found: <%s>")" \
        "$_destpath"
    fi
    ...
    if (( $? )); then
        ms_abort_remove_path "$_fn" "$_remove_build_dir" "$_build_srcdir" "$(gettext "Failure while copying <%s> to <%s>")" \
...
ex_extract_file() {
    ...
    if (( _ret )); then
        ms_abort_remove_path "$_fn" "$_remove_build_dir" "$_build_srcdir" "$(gettext "Failed to extract file <%s> to <%s>")" \
            "$_destpath" "$_build_srcdir"
    fi
    ...
    if [[ -n $_fragment ]]; then
        ...
        case "$_tmp_var" in
            ...
            branch)
                ...
                _ref=origin/${_ref}
                ;;
            *) ms_abort "$_fn" "$(gettext "Unrecognized fragment (reference): '%s'. ENTRY: '%s'")" "$_fragment" "$_entry" ;;
        esac
    fi
...
ex_extract_svn() {
    ...
    if [[ ! -e $_destpath ]]; then
        ms_abort_remove_path "$_fn" "$_remove_build_dir" "$_build_srcdir" "$(gettext "Subversion source  not found: <%s>")" \
        "$_destpath"
    fi
...
ex_extract_hg() {
    ...
    if [[ ! -e $_destpath ]]; then
        ms_abort_remove_path "$_fn" "$_remove_build_dir" "$_build_srcdir" "$(gettext "Mercurial source  not found: <%s>")" \
        "$_destpath"
    fi
