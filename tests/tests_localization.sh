...
ts_lo___lo_generate_pot_file() {
    ...
    te_find_info_msg _COUNT_OK _COUNT_FAILED "${_file_txt}" \
...
ts_lo___lo_generate_po_files() {
    ...
    (lo_generate_po_files _srcfiles _utf8_languages "${_outdir}" "${_pkgname}" "${_copyright}" "${_copyright_start}" \
        "${_url}" "${_bugs_url}") &> /dev/null
    te_retval_0 _COUNT_OK _COUNT_FAILED $? "Test existing source file."
