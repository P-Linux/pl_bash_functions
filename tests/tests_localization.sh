...
u_source_safe_exit "${_FUNCTIONS_DIR}/localization.sh"
...
tsl__l_generate_pot_file() {
    ...
    (l_generate_pot_file "${_wrong_srcfile}" "${_outdir}" "${_pkgname}" "${_copyright}" "${_copyright_start}" "${_url}" \
        "${_bugs_url}") &> /dev/null
    te_retval_1 _COK _CFAIL $? "Test none existing source file."

    (l_generate_pot_file "${_srcfile}" "${_outdir}" "${_pkgname}" "${_copyright}" "${_copyright_start}" "${_url}" \
        "${_bugs_url}")  &> /dev/null
    te_retval_0 _COK _CFAIL $? "Test existing source file."

    _file_txt=$(<"${_outdir}/${_basename}.pot")
    te_find_info_msg _COK _CFAIL "${_file_txt}" \
        "# Package: <pl_bash_functions> autogenerated from source-file: <m" "Test pot file header TITLE."

    te_find_info_msg _COK _CFAIL "${_file_txt}" \
        "# Copyright (C) 2016 - "*"peter1000 <https://github.com/peter1000" "Test pot file header Copyright."
    ...
    te_find_info_msg _COK _CFAIL "${_file_txt}" "Content-Type: text/plain; charset=UTF-8" \
...
tsl__l_generate_po_files() {
    ...
    te_retval_0 _COK _CFAIL $? "Test all expected / generated .pot and .po files exist."
