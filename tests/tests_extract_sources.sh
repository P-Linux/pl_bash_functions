...
ts_ex___ex_extract_source_local_files() {
    ...
    mkdir -p "$(dirname ${_pkgfile_fullpath}2)"
    touch "${_pkgfile_fullpath}2"
    mkdir -p "${_srcdst_dir}2"
    mkdir -p "${_build_srcdir}2"
    cp -f "${_TEST_SCRIPT_DIR}/files/md5sum_testfile.txt" \
        "$(dirname ${_pkgfile_fullpath}2)/md5sum_testfile.txt"
    ...
    _sources=("md5sum_testfile.txt")
    _checksums=("251aadc2351abf85b3dbfe7261f06218")
    so_prepare_src_matrix _scrmtx _sources _checksums "${_pkgfile_fullpath}2" "${_srcdst_dir}2" &> /dev/null

    rm -rf "$(dirname ${_pkgfile_fullpath}2)/md5sum_testfile.txt"
    _remove_build_dir="no"
    _output=$((ex_extract_source _scrmtx "${_build_srcdir}2" "${_remove_build_dir}") 2>&1)
    te_find_err_msg _COUNT_OK _COUNT_FAILED "${_output}" \
