...
ts_so___so_prepare_src_matrix_checksums() {
    ...
    _output=$(so_prepare_src_matrix _scrmtx _sources _checksums "$_pkgfile_fullpath" "$_srcdest_dir")
    te_find_err_msg _COUNT_OK _COUNT_FAILED "$_output" "SRC_CHECKSUMS array size: '2' is greater than SRC_ENTRIES Array size: '1'"
...
ts_so___so_prepare_src_matrix_multiple_source_arrays() {
    ...
    te_retval_0 _COUNT_OK _COUNT_FAILED $? "Test 1. entry (from 1. Pkgfile) all Fields as expected. ENTRY: <${_sources_1[0]}>."
    ...
    te_retval_0 _COUNT_OK _COUNT_FAILED $? "Test 2. entry (from 1. Pkgfile) all Fields as expected. ENTRY: <${_sources_1[1]}>."
    ...
    te_retval_0 _COUNT_OK _COUNT_FAILED $? "Test 3. entry (from 2. Pkgfile) all Fields as expected. ENTRY: <${_sources_2[0]}>."
    ...
    te_retval_0 _COUNT_OK _COUNT_FAILED $? "Test 4. entry (from 2. Pkgfile) all Fields as expected. ENTRY: <${_sources_2[1]}>."
}
ts_so___so_prepare_src_matrix_multiple_source_arrays
...
ts_so___so_prepare_src_matrix_no_srcdst_dir_input() {
    ...
    te_retval_0 _COUNT_OK _COUNT_FAILED $? "Test 1. entry no srcdst dir input all Fields as expected. ENTRY: <${_sources[0]}>."
    ...
    [[ ${_scrmtx[2:ENTRY]} == "resources/mylocal_1.patch"      && \
        ...
        ${_scrmtx[2:DESTNAME]} == "resources/mylocal_1.patch"  && \
        ${_scrmtx[2:DESTPATH]} == "/home/only_download_1/resources/mylocal_1.patch" ]]
    te_retval_0 _COUNT_OK _COUNT_FAILED $? "Test 2. entry no srcdst dir input all Fields as expected. ENTRY: <${_sources[1]}>."
}
ts_so___so_prepare_src_matrix_no_srcdst_dir_input
...
ts_so___so_prepare_src_matrix_general_errors() {
    ...
    _sources=("mylocal.patch")
    _checksums=()
    _output=$((so_prepare_src_matrix _scrmtx_wrong_index_array _sources _checksums "$_pkgfile_fullpath" "$_srcdest_dir") 2>&1)
    te_find_err_msg _COUNT_OK _COUNT_FAILED "$_output" \
